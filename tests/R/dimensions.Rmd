...
The central assumption of the **mumosa** method is that the distance to the $k$-th nearest neighbor is a good proxy for the intra-population variance.
This document examines that assumption with respect to various factors like the number of dimensions in the embedding.
...
# Adding some heteroskedasticity

Now we add some heteroskedasticity across the dimensions.
...
dim <- 1:80
output <- vector("list", length(dim))
actual.var <- numeric(length(dim))

for (i in dim) {
    y <- matrix(rnorm(nobs * i), ncol=i)
    mult <- seq_len(i)
    y <- t(t(y) * mult)
    actual.var[i] <- sum(apply(y, 2, var))
    output[[i]] <- findKNN(y, k = 50, get.index=FALSE)$distance
}
```

We now inspect the median distance at several choices of $k$, as the ratio with the total standard deviation.

```{r}
k <- c(5, 10, 20, 50)
medians <- lapply(k, function(i) vapply(output, function(x) median(x[, i]), 0))
ratios <- lapply(medians, function(x) log2(x/sqrt(actual.var)))
```

Similar results as the i.i.d. case.
Maybe the convergence is a bit slower but it's not too bad.
